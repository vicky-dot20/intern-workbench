...
Complete a reviewable engineering exercise that demonstrates **manage secrets and environment configuration** and explain the decisions you made.

## Context

This is task 6 in the Security phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind manage secrets and environment configuration.
...
1. Create or update the task artifact under `projects/advanced/security-lab/sec-006/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates manage secrets and environment configuration.
...
- Primary workspace: `projects/advanced/security-lab/sec-006/`
- Evidence: `projects/advanced/security-lab/sec-006/README.md`
...
- [ ] The artifact directly demonstrates manage secrets and environment configuration.
...
From the repository root, enter `projects/advanced/security-lab/sec-006` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[SEC-006]`.
...
`task/<github-username>/sec-006-manage-secrets-and-environment-configurati`

## Commit Message

`investigate: complete sec-006 manage secrets and environment configuration`
