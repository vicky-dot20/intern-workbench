...
Complete a reviewable engineering exercise that demonstrates **promises, fetch, and async await** and explain the decisions you made.

## Context

This is task 10 in the JavaScript phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind promises, fetch, and async await.
...
1. Create or update the task artifact under `projects/intermediate/browser-app/js-010/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates promises, fetch, and async await.
...
- Primary workspace: `projects/intermediate/browser-app/js-010/`
- Evidence: `projects/intermediate/browser-app/js-010/README.md`
...
- [ ] The artifact directly demonstrates promises, fetch, and async await.
...
From the repository root, enter `projects/intermediate/browser-app/js-010` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[JS-010]`.
...
`task/<github-username>/js-010-promises-fetch-and-async-await`

## Commit Message

`build: complete js-010 promises, fetch, and async await`
