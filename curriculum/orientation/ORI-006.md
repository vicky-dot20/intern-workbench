...
Complete a reviewable engineering exercise that demonstrates **install and verify the development environment** and explain the decisions you made.

## Context

This is task 6 in the Orientation phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind install and verify the development environment.
...
1. Create or update the task artifact under `notes/orientation.md/ori-006/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates install and verify the development environment.
...
- Primary workspace: `notes/orientation.md/ori-006/`
- Evidence: `notes/orientation.md/ori-006/README.md`
...
- [ ] The artifact directly demonstrates install and verify the development environment.
...
From the repository root, enter `notes/orientation.md/ori-006` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[ORI-006]`.
...
`task/<github-username>/ori-006-install-and-verify-the-development-environ`

## Commit Message

`learn: complete ori-006 install and verify the development environment`
