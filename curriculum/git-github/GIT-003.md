...
Complete a reviewable engineering exercise that demonstrates **stage, commit, log, and diff** and explain the decisions you made.

## Context

This is task 3 in the Git and GitHub phase. It builds one durable skill inside the same GitHub workflow used for product work: issue, branch, implementation, validation, pull request, review, and revision.

## What You Need to Learn

- The core concepts behind stage, commit, log, and diff.
...
1. Create or update the task artifact under `practice/git-lab/git-003/`; do not scatter unrelated changes across the repository.
2. Add a short `README.md` in that task artifact containing the problem, your plan, commands used, and result.
3. Produce the smallest working example or analysis that demonstrates stage, commit, log, and diff.
...
- Primary workspace: `practice/git-lab/git-003/`
- Evidence: `practice/git-lab/git-003/README.md`
...
- [ ] The artifact directly demonstrates stage, commit, log, and diff.
...
From the repository root, enter `practice/git-lab/git-003` and follow its README. If the task is documentation or investigation only, render/preview the Markdown and run the repository validation command instead.
...
1. Confirm the assigned issue title starts with `[GIT-003]`.
...
`task/<github-username>/git-003-stage-commit-log-and-diff`

## Commit Message

`build: complete git-003 stage, commit, log, and diff`
