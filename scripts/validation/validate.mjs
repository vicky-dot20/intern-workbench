...
for(const f of tasks){const text=fs.readFileSync(f,'utf8');const name=path.basename(f,'.md');const id=text.match(/^id: ([A-Z]+-\d{3})$/m)?.[1];if(!id)fail.push(`missing ID: ${f}`);else{if(id!==name)fail.push(`filename/ID mismatch: ${f}`);if(ids.has(id))fail.push(`duplicate ID: ${id}`);ids.add(id)}for(const h of required)if(!text.includes(`## ${h}`))fail.push(`${name}: missing section ${h}`);if(!text.includes('`task/<github-username>/'))fail.push(`${name}: branch name must include <github-username>`);const block=text.match(/prerequisites:\n((?:  - [A-Z]+-\d{3}\n?)*)/);if(block)for(const m of block[1].matchAll(/- ([A-Z]+-\d{3})/g))prereqs.push([name,m[1]]);}
